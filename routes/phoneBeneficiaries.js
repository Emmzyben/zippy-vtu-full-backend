...
// Get all phone beneficiaries for a user
router.get('/', authMiddleware, async (req, res) => {
    try {
        const userId = req.user.id;
        const [beneficiaries] = await db.execute(
            'SELECT id, phone_number, name, created_at FROM phone_beneficiaries WHERE user_id = ? ORDER BY created_at DESC',
            [userId]
        );

        res.json({ success: true, beneficiaries });
    } catch (error) {
        console.error('Get phone beneficiaries error:', error.message);
        res.status(500).json({ success: false, error: 'Failed to fetch beneficiaries' });
    }
});

// Add a new phone beneficiary
router.post('/', authMiddleware, [
    body('phone_number').trim().isMobilePhone().withMessage('Valid phone number is required'),
    ...
    try {
        ...
        const [result] = await db.execute(
            'INSERT INTO phone_beneficiaries (user_id, phone_number, name) VALUES (?, ?, ?)',
...
router.delete('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const [result] = await db.execute(
            'DELETE FROM phone_beneficiaries WHERE id = ? AND user_id = ?',
        ...
        console.error('Delete phone beneficiary error:', error.message);
